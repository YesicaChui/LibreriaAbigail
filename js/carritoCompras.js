function cargarCarritoCompras() {
  ...
  divProductos.innerHTML = `
    ...
    <div class="card m-2">
      ...
      `;
  leerCarrito().then(()=>cargarProductosCarrito())
...
function addEventoPagarCarrito() {
  ...
  btnPagar.addEventListener("click", async() => {
    ...
    if (isAuth()) {
      ...
      const response = await GenerarOrdenPago( GetToken());
      if (response.status === 200) {
        alertPersonalizado("Se genero la orden de compra", true);
        cargarCarritoCompras();
        console.log(response.data.data.checkout_url);
        window.location.href = response.data.data.checkout_url
      } else {
        alertPersonalizado("Error al generar la orden de compra", false);
      }
    } else {
      rejectAuth("Debe Logearse para modificar productos a su carrito");
    }
...
function addEventoEliminar() {
  carrito.forEach((producto, index) => {
    const mibtnBorrar = document.getElementById(`btnBorrar${producto.product.id}`);
    mibtnBorrar.addEventListener("click", async () => {

      if (isAuth()) {

        const response = await DeleteCarrito(producto.product.id, GetToken());
        if (response.status === 200) {
          alertPersonalizado("Se elimino el producto del carrito", true);
          cargarCarritoCompras();
        } else {
          alertPersonalizado("Error al eliminar el producto del carrito", false);
        }
      } else {
        rejectAuth("Debe Logearse para modificar productos a su carrito");
      }
